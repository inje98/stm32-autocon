...
static uint32_t IsI2CMspCbValid = 0;
#endif /* USE_HAL_I2C_REGISTER_CALLBACKS */
static uint32_t I2CInitCounter = 0;
...
int32_t BSP_I2C_IsReady(uint16_t DevAddr, uint32_t Trials)
{
  ...
  if (HAL_I2C_IsDeviceReady(&hi2c3, DevAddr, Trials, BUS_I2C_POLL_TIMEOUT) != HAL_OK)
  {
    ret = BSP_ERROR_BUSY;
  }
...
int32_t BSP_I2C_WriteReg(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  ...
  if (HAL_I2C_Mem_Write(&hi2c3, DevAddr,Reg, I2C_MEMADD_SIZE_8BIT,pData, Length, BUS_I2C_POLL_TIMEOUT) != HAL_OK)
  {
...
int32_t  BSP_I2C_ReadReg(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  ...
  if (HAL_I2C_Mem_Read(&hi2c3, DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length, BUS_I2C_POLL_TIMEOUT) != HAL_OK)
  {
    if (HAL_I2C_GetError(&hi2c3) == HAL_I2C_ERROR_AF)
    {
      ret = BSP_ERROR_BUS_ACKNOWLEDGE_FAILURE;
...
int32_t BSP_I2C_WriteReg16(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  ...
  if (HAL_I2C_Mem_Write(&hi2c3, DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length, BUS_I2C_POLL_TIMEOUT) != HAL_OK)
  {
    if (HAL_I2C_GetError(&hi2c3) == HAL_I2C_ERROR_AF)
    {
      ret = BSP_ERROR_BUS_ACKNOWLEDGE_FAILURE;
...
int32_t  BSP_I2C_ReadReg16(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  ...
  if (HAL_I2C_Mem_Read(&hi2c3, DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length, BUS_I2C_POLL_TIMEOUT) != HAL_OK)
  {
    if (HAL_I2C_GetError(&hi2c3) != HAL_I2C_ERROR_AF)
    {
      ret =  BSP_ERROR_BUS_ACKNOWLEDGE_FAILURE;
...
int32_t BSP_I2C_Send(uint16_t DevAddr, uint8_t *pData, uint16_t Length) {
  ...
  if (HAL_I2C_Master_Transmit(&hi2c3, DevAddr, pData, Length, BUS_I2C_POLL_TIMEOUT) != HAL_OK)
  {
    if (HAL_I2C_GetError(&hi2c3) != HAL_I2C_ERROR_AF)
    {
      ret = BSP_ERROR_BUS_ACKNOWLEDGE_FAILURE;
...
int32_t BSP_I2C_Recv(uint16_t DevAddr, uint8_t *pData, uint16_t Length) {
  ...
  if (HAL_I2C_Master_Receive(&hi2c3, DevAddr, pData, Length, BUS_I2C_POLL_TIMEOUT) != HAL_OK)
  {
    if (HAL_I2C_GetError(&hi2c3) != HAL_I2C_ERROR_AF)
    {
      ret = BSP_ERROR_BUS_ACKNOWLEDGE_FAILURE;
...
#if (USE_HAL_I2C_REGISTER_CALLBACKS == 1U)
/**
  * @brief Register Default BSP I2C Bus Msp Callbacks
  * @retval BSP status
  */
int32_t BSP_I2C_RegisterDefaultMspCallbacks (void)
{
  ...
  if (HAL_I2C_RegisterCallback(&hi2c3, HAL_I2C_MSPINIT_CB_ID, I2C_MspInit)  != HAL_OK)
  {
    return BSP_ERROR_PERIPH_FAILURE;
  }

  /* Register MspDeInit Callback */
  if (HAL_I2C_RegisterCallback(&hi2c3, HAL_I2C_MSPDEINIT_CB_ID, I2C_MspDeInit) != HAL_OK)
  {
    return BSP_ERROR_PERIPH_FAILURE;
  }
  IsI2CMspCbValid = 1;
...
/**
  * @brief BSP I2C Bus Msp Callback registering
...
static void I2C_MspInit(I2C_HandleTypeDef* i2cHandle)
{
  ...
  /* USER CODE END I2C_MspInit 0 */
    ...
    PA10     ------> I2C_SDA
    */
    GPIO_InitStruct.Pin = BUS_I2C_SCL_GPIO_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_OD;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    GPIO_InitStruct.Alternate = BUS_I2C_SCL_GPIO_AF;
    HAL_GPIO_Init(BUS_I2C_SCL_GPIO_PORT, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = BUS_I2C_SDA_GPIO_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_OD;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    GPIO_InitStruct.Alternate = BUS_I2C_SDA_GPIO_AF;
    HAL_GPIO_Init(BUS_I2C_SDA_GPIO_PORT, &GPIO_InitStruct);
...
static void I2C_MspDeInit(I2C_HandleTypeDef* i2cHandle)
{
  ...
  /* USER CODE END I2C_MspDeInit 0 */
    ...
    PA10     ------> I2C_SDA
    */
    HAL_GPIO_DeInit(BUS_I2C_SCL_GPIO_PORT, BUS_I2C_SCL_GPIO_PIN);

    HAL_GPIO_DeInit(BUS_I2C_SDA_GPIO_PORT, BUS_I2C_SDA_GPIO_PIN);
